...
import { AVAILABLE_INDICATORS } from "@/components/CustomStrategyModal";
import {
    Dialog,
    DialogContent,
    DialogHeader,
...
export default function StrategyBacktestMonitor({
    ...
    if (stats.totalSignals === 0) return (
        <div className="text-xs text-gray-500 italic">Yeterli sinyal bulunamadı</div>
    );
            ...
            <div className="flex flex-col gap-1">
                ...
                <div className="flex items-center gap-2 mt-0.5">
                    <span className="text-[11px] text-gray-300 font-medium bg-gray-800 px-2 py-0.5 rounded border border-gray-700">
                        {stats.totalSignals} Sinyal
                    </span>
                    <span className="text-[11px] text-gray-400">{stats.wins} Hit</span>

                    <Dialog>
                        <DialogTrigger asChild>
                            <button className="ml-auto p-1 hover:bg-violet-900/40 rounded transition-colors text-violet-400 hover:text-violet-200" title="İşlem Geçmişi">
